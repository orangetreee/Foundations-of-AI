    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos, oldProb in self.beliefs.items():
            for newPos, newProb in self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)).items():
                newPoses[newPos] += oldProb * newProb
        newPoses.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for index, p in enumerate(self.particles):
            self.particles[index] = util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, p)))
        # util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [source[i % len(source)] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        for index, distance in enumerate(noisyDistances):
            if distance is None:
                self.particles = [self.getParticleWithGhostInJail(p, index) for p in self.particles]

        beliefs = util.Counter()
        for p in self.particles:
            prob = 1
            for index, ghost, model, distance in zip(range(len(p)), p, emissionModels, noisyDistances):
                if distance is None:
            ...
            for index in range(len(newParticle)):
                newParticle[index] = util.sample(
                    getPositionDistributionForGhost(
                        setGhostPositions(gameState, oldParticle), index, self.ghostAgents[index]
                    )
                )
